<script lang="ts">
	...
	const radioImport = import.meta.glob<RadioModule>('$lib/_content/radio/*.md', { eager: true });
	const episodes: Array<RadioEpisode> = Object.entries(radioImport)
		.map(([path, mod]: [string, RadioModule]) => {
...
<div class="main-content w-full !px-2 !py-10 sm:!px-4 lg:!px-20">
	...
	>
		{#each episodes as file (file.slug)}
			<button
